...
namespace CompanionDisplayWinUI.ClassImplementations
{
    class UpdateSystem
    {
        public async static void PerformUpdate(bool downloadUpdate)
        {
            ...
            if(updatePath != "")
            {
                CMDOperations.PerformCMDCommand("taskkill /f /im CompanionDisplayWinUI.exe & mkdir Update & MOVE * Update/ & cd Update & move CompanionDisplayWinUI.exe.WebView2 ../ & move Config ../ & move \"" + updatePath + "\" ../release.zip & move setup.exe ../ & cd .. & tar -xf release.zip & del /f /q release.zip & rmdir /s /q Update & CompanionDisplayWinUI.exe");
            }
        }
